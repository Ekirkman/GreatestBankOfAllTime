...
<a class="btn btn-primary" href="OpenAccountServelet" role="button" >Open new Account</a>
 <button type="button" class="btn btn-danger" href="CloseSecondaryServlet">Close This Account</button>
<h2> Account Name: </h2>
<h3><%out.print(customer.getAccounts().get(1).getAccountName()); %></h3>
<h2>Balance:</h2>
<h2><%out.print(customer.getAccounts().get(1).getBalance()); %></h2>
...
<table class="table">
  ...
  <tbody>
	<c:forEach var="trans" items="${customer.getAccounts().get(1).getStatement()}">
    <tr>
